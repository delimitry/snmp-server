...
snmpget_command = '/usr/bin/snmpget -Oq -v2c -t 1 -c public 127.0.0.1:{port}'
snmpset_command = '/usr/bin/snmpset -Oq -v2c -t 1 -c public 127.0.0.1:{port}'
snmpwalk_command = '/usr/bin/snmpwalk -Oq -v2c -t 1 -c public 127.0.0.1:{port}'


def test_request_replies_correctly(snmpserver):
    snmpserver.expect_request("1.3.6.1.2.1.2.2.1.2", "qqq")
    p = subprocess.Popen(shlex.split(f'{snmpget_command.format(port=snmpserver.port)} IF-MIB::ifDescr'), stdout=subprocess.PIPE)
    p.wait()
...
def test_dual_request_replies_correctly(snmpserver):
    ...
    p = subprocess.Popen(shlex.split(f'{snmpget_command.format(port=snmpserver.port)}  IF-MIB::ifOperStatus.1005 IF-MIB::ifDescr.1004'), stdout=subprocess.PIPE)
    p.wait()
...
def test_snmp_range_correctly(snmpserver):
    ...
    p = subprocess.Popen(shlex.split(f'{snmpset_command.format(port=snmpserver.port)}  IF-MIB::ifAdminStatus.1004 i 7'),
...
def test_snmp_walk(snmpserver):
    ...
    p = subprocess.Popen(shlex.split(f'{snmpwalk_command.format(port=snmpserver.port)}  IF-MIB::ifIndex'),
                         stdout=subprocess.PIPE,
                         stderr=subprocess.PIPE)
    p.wait()
    stdout = p.stdout.read().decode('utf-8')
